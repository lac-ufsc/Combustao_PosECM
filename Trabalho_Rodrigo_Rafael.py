...
cti_def = """ideal_gas(name='gri30' , elements= ' C H O N',
          ...
          initial_state=state(temperature=500,pressure=101325))"""

T = [298, 600, 900]
P = [ct.one_atm, 5*ct.one_atm,10*ct.one_atm]
Phi = [0.75,0.9,1.00,1.05,1.1,1.25]
...
## variáveis
Tad = np.zeros((len(T),len(P),len(Phi)))
xCO2 = np.zeros((len(T),len(P),len(Phi)))
